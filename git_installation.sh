...
get_git_install_location(){
  ...
  then
    ...
    mv $git_path/git $git_path/git_backup
    echo $git_path
  else
    echo '/usr/local'
  fi
...
then
   ...
   if [ -e $git_path/git_backup ]
   then
     $git_path/git_backup $git_path/git
   fi
fi
}
...
main(){
 prHeader "="
 prtxtCentre "WELCOME TO BASH SHELL SCRIPTING"
 prHeader "="
...
if [ "$USER" != "root" ]
then
  prRed "$(get_date_time): please run the shell script with root user"
  exit 1
else
  main
fi
